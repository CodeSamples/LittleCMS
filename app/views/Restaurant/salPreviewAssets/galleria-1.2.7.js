    ...
    var lightbox = this._lightbox = {
        ...
        init : function() {
            ...
            var elems = 'overlay box content shadow title info close prevholder prev nextholder next counter image',
                ...
                cssMap = {
                    ...
                    info:       abs+'bottom:10px;left:10px;right:10px;color:#444;font:11px/13px arial,sans-serif;height:auto',
                    close:      abs+'top:10px;right:10px;height:20px;width:20px;background:#fff;text-align:center;cursor:pointer;color:#444;font:16px/22px arial,sans-serif;z-index:99999',
                    image:      abs+'top:10px;left:10px;right:10px;bottom:30px;overflow:hidden;display:block;',
                    prevholder: abs+'width:50%;top:0;bottom:40px;cursor:pointer;',
                    nextholder: abs+'width:50%;top:0;bottom:40px;right:-1px;cursor:pointer;',
                    prev:       abs+'top:0;height:100%;width:30px;left:0px;display:none;text-align:center;color:#000;font:bold 16px/36px arial,sans-serif',
                    next:       abs+'top:0;height:100%;width:30px;right:0px;left:auto;display:none;font:bold 16px/36px arial,sans-serif;text-align:center;color:#000',
                    title:      'float:left;width:94%;word-wrap:break-word;',
        ...
        rescale: function(event) {
            ...
            if ( event ) {
                ...
                /* Adjust image size according to info box height */
                $(lightbox.image.image).css({ height: lightbox.elems.image.clientHeight - lightbox.elems.info.clientHeight });
            } else {
                $( lightbox.elems.box ).animate( to, {
                    ...
                    complete: function() {
                        /* Adjust image size according to info box height */
                        $(lightbox.image.image).css({ height: lightbox.image.container.clientHeight - lightbox.elems.info.clientHeight });
        ...
        show: function(index) {
            ...
            if(typeof data.iframe != 'undefined')
                data.iframe = data.iframe.replace('gallery', 'full');
*/
            Utils.hide( lightbox.elems.info );
            ...
            if(typeof lightbox.image != 'undefined')
            ...
            if(typeof data.iframe != 'undefined')
            {
                lightbox.image.load( data.iframe.replace('gallery', 'full') || data.big || data.image, function( image ) {

                    lightbox.width = image.isIframe ? 740 : image.original.width;
                    lightbox.height = image.isIframe ? 500 : image.original.height;
    ...
    _createThumbnails : function() {
        ...
        for( i = 0; this._data[ i ]; i++ ) {
            ...
            if ( o.thumbnails === true && (data.thumb || data.image) ) {
                ...
                var videoUrl = 'http://assets.video.gfrmedia.com/';
                var videoRegExp = new RegExp(videoUrl, 'gi');
                var playHtml = '<div class="videoPlaySymbol"></div>';

                // append the thumbnail
                this.$( 'thumbnails' ).append( thumb.container );

                if(videoRegExp.test(src))
                    jQuery(thumb.container).append(playHtml);
